...
export default class Content extends Component {
    ...
    componentDidMount() {
        ...
        // const path_id = "45"
        var path_link = "http://localhost:5000/paths/" + path_id // json-server
        var course_link = "http://localhost:5000/courses" // json-server
        ...
        axios.get(course_link) // change this url to whichever end point to use
          ...
          // .then(response => this.setState({course: response.data._embedded.courses}))
    }
    ...
    render () {
        ...
        for (var i=0; i<this.state.course.length;i++){
            if (course_ids.includes(this.state.course[i].id)) // Json-server
                    ...
                    return (
                        <Col md={4}>
                            <Card style={{ width: "100%" }}>
                              <Card.Img variant="top" src={course.image_link}/> {/*json-server */}
